            ...
            <SideBar />
            {
                writer !== '' ? (
                    <div className='bg-nomad-sand min-h-full flex flex-col justify-center items-center pt-12'>
                        <label htmlFor='file' 
                            className={`block relative w-[60%] pb-[60%] bg-white ${state === 1 ? '' : 'cursor-pointer'} shadow-full mb-5`}>
                            <img src={process.env.REACT_APP_IMG_URL + imgKey} alt='doc-pic' className='absolute w-full h-full object-cover'/>
                            <input type='file' id='file' accept='image/*' className='hidden' onChange={(e)=>changeFileImg(e)} disabled={state === 1}/>
                        </label>
                        <input type='text' maxLength={10} placeholder='습득 위치 입력' className='bg-transparent font-nexonBold text-2xl focus:outline-none w-[70%] text-center' 
                            disabled={state === 1} value={title} onChange={(e)=>setTitle(e.target.value)}/>
                        ...
                        <div css={leftDiv}>
                            ...
                            <textarea css={gridDiv} placeholder='분실물 보관 장소를 적어주세요.' className='block w-full focus:outline-none bg-transparent h-24 mb-3 resize-none' 
                                disabled={state === 1} value={descript} onChange={(e)=>setDescript(e.target.value)}/>
                        </div>
                        {state !== 1 && (
                            <div>
                                <button type='button' css={token} className='mx-1' onClick={()=>registButtonClick(params, setState, nav)}>등록</button>
                                <button type='button' css={token} className='mx-1' onClick={()=>state ? setState(1) : nav('/lost')}>취소</button>
                            </div> /* 글 등록 시 */
                        )}
                        {state === 1 && mine === true && (
                            <div>
                                <button type='button' css={token} className='mx-1' onClick={()=>setState(2)}>수정</button>
                                <button type='button' css={token} className='mx-1' onClick={()=>deleteButtonClick(postId, imgKey, nav)}>삭제</button>
                            </div> /* 내 글 상세 페이지 */
                        )}
                        {state === 1 && mine === false && (
                            <button type='button' css={token} onClick={()=>nav('/lost')}>목록</button> /* 일반 글 상세 페이지 */
                        )}
                    </div>
                ) : <LostFormSkeleton />
            }
