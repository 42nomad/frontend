...
function LostDoc({ isMyPage, docData }: LostDocProps) {
	...
	return (
		<div
			className={`text-center cursor-pointer bg-white rounded-xl p-3 ${isMyPage ? 'pb-0' : ''} shadow-full`}
			onClick={() => lostDocClick(nav, postId)}
			aria-hidden
		>
			<div className="relative pb-[100%] mb-2">
				<img src={imgKey} className="absolute object-cover w-full h-full rounded-lg" alt="lost-img" />
			</div>
